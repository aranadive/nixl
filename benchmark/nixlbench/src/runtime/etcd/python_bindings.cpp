...
PYBIND11_MODULE(etcd_runtime, m) {
    ...
    py::class_<xferBenchEtcdRT>(m, "EtcdRuntime")
        .def(py::init<const std::string&, const int, int*>(),
             py::arg("etcd_endpoints"), py::arg("size"), py::arg("terminate") = nullptr)
        .def("get_rank", &xferBenchEtcdRT::getRank)
        .def("get_size", &xferBenchEtcdRT::getSize)
        .def("send_int", [](xferBenchEtcdRT& self, int value, int dest_rank) {
            return self.sendInt(&value, dest_rank);
        })
        .def("recv_int", [](xferBenchEtcdRT& self, int src_rank) {
            int value;
            int result = self.recvInt(&value, src_rank);
            return py::make_tuple(result, value);
        })
        .def("send_char", [](xferBenchEtcdRT& self, const std::string& data, int dest_rank) {
            return self.sendChar(const_cast<char*>(data.c_str()), data.size(), dest_rank);
        })
        .def("recv_char", [](xferBenchEtcdRT& self, size_t count, int src_rank) {
            std::vector<char> buffer(count);
            int result = self.recvChar(buffer.data(), count, src_rank);
            std::string data(buffer.begin(), buffer.end());
            return py::make_tuple(result, data);
        })
        .def("broadcast_int", [](xferBenchEtcdRT& self, py::array_t<int> buffer, int root_rank) {
        ...
        .def("reduce_sum_double", [](xferBenchEtcdRT& self, double local_value, int dest_rank) {
            double global_value;
